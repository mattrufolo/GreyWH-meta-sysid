...
if __name__ == '__main__':
    ...
    try:
        ...
        def loss_fn(p, u, y, x1= None, x2= None):
            ...
            y_hat = functional_call(wh, p, args = (u))
            y_hat = (y_hat-y.mean(axis = 0))/(y.std(axis = 0)+1e-6)
            y = (y-y.mean(axis = 0))/(y.std(axis = 0)+1e-6)
        ...
        def batched_loss_fn(pm, batch_u2,batch_y2):#,batch_x2= None
            ...
            return torch.mean(loss)#,batch_x2
        ...
        def estimate_loss(dl,params):
            ...
            for eval_iter, (batch_y, batch_u, par) in enumerate(dl):
                ...
                if device_type == "cuda":
                    ...
                    #     val_loss.backward()
                    #     inner_opt.step()
                    val_loss = batched_loss_fn(wh_params, batch_u_dec, batch_y_dec)
        ...
        for iter_num_new, (batch_y, batch_x, par) in tqdm.tqdm(enumerate(train_dl, start=iter_num_new)):
            ...
            batch_x2 = batch_x[:, cfg.seq_len_ctx-50:,:]
            batch_y2 = batch_y[:, cfg.seq_len_ctx-50:,:]
            ...
            loss = batched_loss_fn(wh_params_detached, batch_x2, batch_y)
